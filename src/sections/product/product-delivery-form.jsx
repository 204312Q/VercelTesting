...
export function ProductDeliveryForm({
  ...
  const validateForm = useCallback(() => {
    ...
    const isFormValid = Object.keys(validationRules).every(
      (field) => !validationRules[field](deliveryData[field]),
    );
  ...
  const isPartialPaymentAvailable = Number(basePrice) >= 728;

  useEffect(() => {
    if (deliveryData.paymentMethod === 'partial' && !isPartialPaymentAvailable) {
      setDeliveryData((prev) => ({ ...prev, paymentMethod: 'full' }));
    }
  }, [isPartialPaymentAvailable, deliveryData.paymentMethod]);
