export async function getBundlesForProduct(productId, duration) {
  try {
    const url = `/api/public/packages/${productId}/bundles?duration=${encodeURIComponent(
      duration
    )}`;

    const res = await fetch(url, { cache: 'no-store' });
    if (!res.ok) {
      console.error('getBundlesForProduct: HTTP', res.status);
      return [];
    }
